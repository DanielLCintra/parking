...
export default {
  ...
  computed: {
    ...
    selected () {
      ...
      this.$emit('selected', {value: this.values, sel: sel,name: this.name})
      return sel.join(', ')
    },
    ...
    select (v) {
      if (this.val instanceof Array) {
        ...
        if (~newVal.indexOf(v)) {
          ...
          this.$bus.$emit('remove-item',v)
        } else {
          newVal.push(v)
          this.$bus.$emit('change-tab',v)
        }
        this.val = newVal
  ...
  mounted () {
    ...
    this.$bus.$on('set-checked', obj => {
      this.val = obj.val
    })

    this.$bus.$on('clear-all', obj => {
      this.val = []
    })
